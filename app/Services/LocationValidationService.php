...
{
    // القيم الافتراضية لحدود دبي (للتوافق مع البيانات القديمة)
    ...
    /**
     * الحصول على جميع الحدود الجغرافية من قاعدة البيانات
     */
    private static function getAllBounds()
    {
        return GeographicalBound::all();
    }

    /**
     * الحصول على حدود دبي من الإعدادات (للتوافق مع البيانات القديمة)
     */
    ...
    /**
     * التحقق من أن الموقع داخل أي من الحدود الجغرافية
     */
    public static function isWithinAnyBound(float $latitude, float $longitude): bool
    {
        ...
        \Log::info('Location validation check - not within any bound', [
            'latitude' => $latitude,
            'longitude' => $longitude,
            'total_bounds' => $bounds->count(),
        ]);
    ...
    /**
     * التحقق من أن الموقع داخل حدود دبي (للتوافق مع البيانات القديمة)
     */
    public static function isWithinDubai(float $latitude, float $longitude): bool
    {
        $bounds = self::getDubaiBounds();
        
        \Log::info('Location validation check (legacy)', [
            'latitude' => $latitude,
            'longitude' => $longitude,
            'bounds' => $bounds,
            'lat_check' => [
        ...
        \Log::info('Location validation result (legacy)', [
            'latitude' => $latitude,
            'longitude' => $longitude,
    ...
    {
        if (!self::isWithinAnyBound($latitude, $longitude)) {
            return [
                'valid' => false,
